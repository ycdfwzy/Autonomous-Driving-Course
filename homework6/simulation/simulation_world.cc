...
void SimulationWorld::Initialize() {
  AddVehicleAgent(simulation_config_.agent_name());
  //fout.open("/home/ycdfwzy/Desktop/velocity.txt");
}
...
void SimulationWorld::UpdateVehicleStatus() {
  ...
  for (auto& iter : agent_status_map_) {
    ...
    if (simulation_status.is_alive()) {
      ...
      //fout << CalcVelocity(agent_status_map_.at(name).vehicle_status().velocity()) << std::endl;
