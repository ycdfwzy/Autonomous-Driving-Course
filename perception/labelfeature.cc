...
const std::string path_prefix = "/home/ycdfwzy/autonomous_driving/PublicCourse/perception_project/20180103_nanian_103247";
...
inline void swap_point(Point3D* p1, Point3D* p2){
    ...
    p3->set_x(p1->x()); p3->set_z(p1->z()); p3->set_z(p1->z());

    p1->set_x(p2->x()); p1->set_z(p2->z()); p1->set_z(p2->z());
    p2->set_x(p3->x()); p2->set_z(p3->z()); p2->set_z(p3->z());
}
